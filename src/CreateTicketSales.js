...
import { Box, Button, Grid, TextField, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Select, CircularProgress } from '@material-ui/core';
...
export default function CreateTicketSales() {
  ...
  const [nasabah, setNasabah] = React.useState("");  
  const [content, setContent] = React.useState([]);
  
  const [options, setOptions] = React.useState([]);
  const [selectedValue, setSelectedValue] = React.useState('');
  const [loading, setLoading] = React.useState(true);
  const [approvelimitMap, setApproveLimitMap] = React.useState(new Map());
  ...
  const fetchOptions = async () => {
    try {
      axios.get(backendUrl + "/useraccount/" + user_id, {
        headers: {
        ...
        setOptions(data.map((item) => ({ label: item.name, id: item.id })));
        const tasksMap = new Map(data.map((task) => [task.id, task]));
        setApproveLimitMap(tasksMap)
    ...
    <div className={classes.root}>
      ...
      <Grid item xs={12}>
          <Box component="span" display="block" border={1}>
            <div>
              <Typography>Please select Customer</Typography>
              <Autocomplete
              ...
              onChange={(event, newValue) => {
                const task = approvelimitMap.get(newValue.id);
                setCurrentApproveLimit(parseInt(task.approvelimit, 10).toLocaleString())
                console.log(task.id+" "+task.approvelimit);
                console.log("Selected Option:", newValue);
                setCustcode(newValue.id)
                }}                
              />
            </div>
          </Box>
